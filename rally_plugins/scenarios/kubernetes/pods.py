    ...
    def _parse_pod_status_conditions(self, conditions):
        ...
        for d in conditions:
            if d.type == "PodScheduled":
                ...
                stat_dict["kubernetes.scheduled_pod"].update({"finished_at": ready})
            elif d.type == "Initialized":
                init_time = to_time(d.last_transition_time.timetuple())
                stat_dict["kubernetes.ready_pod"].update({"finished_at": init_time})
                ...
                stat_dict["kubernetes.initialized_pod"].update({"finished_at": finish})
